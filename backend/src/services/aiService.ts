...
import { getConversationContext, createConversationContext, addQuestionToContext, getContextualPrompt } from './conversationService';
import { generateContentWithGemini } from './geminiService';
 ...
 */
export async function answerQuestionAboutResume(
  question: string, 
  resume: ParsedResume,
  sessionId?: string
): Promise<AIResponse> {
  ...
  try {
    ...
    let context = sessionId ? getConversationContext(sessionId) : null;
    if (!context && sessionId) {
      context = createConversationContext(sessionId, resume);
    }
    
    // Create a comprehensive context from the resume
    const resumeContext = createResumeContext(resume);
    const contextualInfo = context ? getContextualPrompt(context, resume) : '';
...
Remember: You are analyzing a real person's career. Be respectful, accurate, and helpful.`;
    ...
    const geminiResponse = await generateContentWithGemini(fullPrompt);
    const answer = geminiResponse.answer;
    
    // Add question and answer to conversation context
    if (context && sessionId) {
      addQuestionToContext(sessionId, question, answer);
    }
    
    return {
      answer,
      confidence: 0.9, // High confidence for AI responses
      sources: ['resume_content', 'ai_analysis']
    };
    ...
    // Known scenario: unpaid accounts or model access errors
    // Fallback to rule-based answering if OpenAI fails
    return fallbackAnswer(question, resume);
  }
}

function createResumeContext(resume: ParsedResume): string {
...
function fallbackAnswer(question: string, resume: ParsedResume): AIResponse {
  ...
  if (q.includes('email') || q.includes('contact')) {
    const contactInfo = [];
    if (structuredData.email) contactInfo.push(`Email: ${structuredData.email}`);
    if (structuredData.phone) contactInfo.push(`Phone: ${structuredData.phone}`);
  ...
  if (q.includes('education') || q.includes('degree') || q.includes('university') || q.includes('college')) {
    ...
    if (education.length > 0) {
      ...
      education.forEach((edu, index) => {
        ...
        if (edu.institution) response += ` from ${edu.institution}`;
        if (edu.year) response += ` (${edu.year})`;
        response += '\n';
      });
      return {
        answer: response.trim(),
